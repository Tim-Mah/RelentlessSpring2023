...
public class Level2PlayerScript : MonoBehaviour
{
    ...
    private float coldStartRunBoost = 6.1f;
    ...
    private Vector2 wallJumpPower = new Vector2(25f, 18f);
    ...
    private void MoveCode()
    {
        ...
        // Player ground movement
        {
            ...
            else
            {
                ...
                if (run)
                {
                    rb2.AddForce(movementDirection * runSpeed * 4 * Time.fixedDeltaTime);
                    wasRunnging = true;
                }
                else
                {
                    rb2.AddForce(movementDirection * walkSpeed * 4 * Time.fixedDeltaTime);
                    wasRunnging = false;
                }
            }
        }
    ...
    {
        if (movable != null && onMovable && horizontal == 0 && IsGrounded() && !isSliding && !crouch)
        {
    ...
    void Update()
    {
        ...
        if (IsGrounded())
        {
            ...
            {
                accelBoost = coldStartRunBoost * 0.1f;
            }
            if (!isDashing)
            {
                canDash = true;
